...
const CLOUD_COLLECTIONS = new Set(["azure", "aws", "gcp"]);

export function buildIconsets(iconData) {
  const byIconset = new Map();

  for (const icon of iconData) {
    const collection = CLOUD_COLLECTIONS.has(icon.collection)
      ? icon.collection
      : undefined;
    const variants = Object.entries(icon.variants ?? {});
    for (const [variant, iconPath] of variants) {
      if (!collection && variant === "default" && variants.length !== 1) continue;
      const normalizedPath = iconPath.startsWith("/") ? iconPath : `/${iconPath}`;
      const iconset = collection
        ? variant === "default"
          ? collection
          : `${collection}-${variant}`
        : variant;
      const icons = byIconset.get(iconset) ?? [];
      icons.push({
        name: icon.title,
        url: `${RAW_PUBLIC_BASE}${normalizedPath}`,
        license: icon.license ?? "Unknown",
      });
      byIconset.set(iconset, icons);
    }
  }

  return [...byIconset.entries()]
    .toSorted(([a], [b]) => a.localeCompare(b))
    .map(([iconset, icons]) => ({
      variant: iconset,
      fileName: `${iconset}.json`,
      data: {
        name: `Unofficial theSVG Iconset - ${iconset}`,
        description: `xream 基于 theSVG 转换的 ${iconset} variant 图标集；仅包含链接清单，不重新分发 SVG。图标版权、商标和 license 归各自权利方及 upstream license 约束。`,
